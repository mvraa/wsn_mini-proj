...
static AggregatedData* AggDataPackage[AGG_DATA_PACKAGE_LENGTH]; //This stores the aggrated data packages 
         ...
         uint16_t datalen)
{
  ...
  AggregatedData *agg_data = (AggregatedData *) data;
  printf("Received: \n");
  printf("\t avgTemp: ");
  printf_float(agg_data->avgTemp, 3);
  printf("\n\t avgHum: ");
  printf_float(agg_data->avgHum, 3);
  printf("\n\t avgPhoto: ");
  printf_float(agg_data->avgPhoto, 3);
  printf("\n\t avgSolar: ");
  printf_float(agg_data->avgSolar, 3); 
  rx_count++;
